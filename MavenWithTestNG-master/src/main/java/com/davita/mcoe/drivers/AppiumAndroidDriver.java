...
public class AppiumAndroidDriver {
	...
	public AndroidDriver<?> getDriver(ExecutionAgent executionAgent, AppMonitor monitor) {
			...
			if ("emulator".equals(DEVICETYPE) || "real".equals(DEVICETYPE)) {
				...
				try {
					if (HTTPClient.isHostReachable(appiumUrl + "/status")) {
						driver = new AndroidDriver(new URL("http://" + APPIUMSERVER + ":" + APPIUMPORT + "/wd/hub"),
								capabilities);
					} else {
					...
					LOGGER.error(Utilities.getCurrentThreadId() + "Exception in intializing the Android Appium Driver."
