...
public class Reporter {
	...
	private static final String[] HEADER = { "Test Case ID",
			"Test Name and Steps", "Status", "Comments" };
	...
	static {
		...
		String text = "<html>"
				...
				+ "http://www.citiustech.com/images/logo.gif"
		...
		try {
			...
			throw new RuntimeException(
					"IOException in the static block of Reporter Class", io);
		} catch (Exception ex) {
			throw new RuntimeException(
					"Exception in the static block of Reporter Class", ex);
		}
	}
		...
		if (colLength > 0) {
			...
			try {
				...
				while (i < arrHeader.length) {
					...
					reportContent2 = reportContent2 + "<th class=bborder_left>"
							+ arrHeader[i] + "</th>";
				...
				throw new IOException(
						"IOException in the addHeader() method of Reporter Class",
						io);
			}
		}
	}
		...
		try {
			...
			if (colLength > 0) {
				...
				while (i < arrData.length) {
					...
					reportContent2 = reportContent2 + "<td class=normal_text>"
							+ arrData[i] + "</td>";
					outputReport.write(reportContent2);
					reportContent2 = "";
					i = i + 1;
			...
			throw new IOException(
					"IOException in the addData() method of Reporter Class", io);
		}
	...
	public static void closeReport() throws IOException {
		try {
			...
			throw new IOException(
					"IOException in the closeReport() method of Reporter Class",
					io);
		}
	}
}
